...
def get_projetos():
    ...
    control_issue_number = control_issues.get("issue_corrente")
    control_issue_project = control_issues.get("projeto_corrente")
    control_page = control_issues.get("page")
    isNextPage = control_issues.get("nextPage")
    projects = readJson("projects", "database/projects/")
    issue_target = False
    nextPage = False
    for author in autores_toxico:
        current_author = author
        ...
        while totalCount ==1000 and not limit_exceeded :
            try: 
                ...
                for issue in issues:
                    ...
                    print(f"{issue.repository.name} {issue.repository.owner.login}")
                    projects[issue.repository.name] = issue.repository.owner.login
                    writeDictToJson(projects,"projects", "database/projects/")
                ...
                difference = set(autores_toxico).difference(autores_analisados)
                control_issues["autores_toxico"] = sorted(list(difference))
                control_issues["page"] = page if nextPage else 0 
                control_issues["nextPage"] = nextPage
                writeDictToJson(control_issues,"control_issues", "database/issues/")
                break
            except Exception as e:
                erro = True
        ...
        difference = set(autores_toxico).difference(autores_analisados)
        control_issues["autores_toxico"] = sorted(list(difference))
        control_issues["page"] = 0 
        control_issues["nextPage"] = False
        writeDictToJson(control_issues,"control_issues", "database/issues/")
    if(not erro):
        print("sem erros")
        difference = set(autores_toxico).difference(autores_analisados)
